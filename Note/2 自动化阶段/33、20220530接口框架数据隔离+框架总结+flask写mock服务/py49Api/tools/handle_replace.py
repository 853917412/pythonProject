...
class HandleReplace:
    ...
    def setup_sql(self,setup_sql):
        ...
        """
        try:
            myLog.info(msg="--开始执行前置sql--")
            myLog.info(msg=f"--前置sql入参setup_sql:{setup_sql},数据类型{type(setup_sql)}--")
    ...
    def replace_sql(self,sql:str):
        ...
        if len(key_list) > 0:
            myLog.info(msg="--需要替换sql语句--")
            myLog.info(msg=f"--替换前sql语句{sql}--")
            # 将需要替换的数据设置为类属性，用来替换
            for key in key_list:
                # 获取数据并设置为类属性
                self.get_data_and_set_attr(key=key)
            #2、替换sql语句
            for key in key_list:
                myLog.info(msg=f"sql语句替换的key：{key}")
                myLog.info(msg=f"sql语句替换通过key：{key}找到的全局属性的值:{getattr(HandleAttribute,key)}")
                sql = sql.replace(f"#{key}#",str(getattr(HandleAttribute,key)))
        else:
            myLog.info(msg="不需要替换sql语句直接返回")
        myLog.info(msg=f"--替换后sql语句{sql}--")
        return sql
