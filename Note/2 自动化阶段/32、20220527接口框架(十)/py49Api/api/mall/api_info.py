...
秒杀商品测试
功能：
1、秒杀时间设置(在可以秒杀的时间范围内)
2、限购数量(边界值，一个id多次购买)
3、付款时间(边界值)
4、超时支付之后库存怎么处理(1、恢复库存继续卖  2、不恢复库存)
5、同一个订单并发过去购买(幂等：只能成交一个)
6、抢库存下单接口(10个商品20个人同时购买，成交规则，锁库存，成交队列)
   按下单时间---按用户等级---用户id大小(id越小注册越早)---先让新用户成交
7、模拟回调：看自己系统怎么处理这些订单和库存(订单各种状态流转、支付成功、支付失败、自动取消支付、风控取消)
8、主动查询(查到订单的交易状态，同步自己系统的数据：订单各种状态流转、支付成功、支付失败、自动取消支付、风控取消)
性能：
1、同时支持多少人一起秒杀
2、一秒钟一般接口限制值允许6次访问
安全：
1、越权下单
2、数据越权
3、支付接口数据要加密传输（个人信息、支付信息、）
4、数据库日志敏感信息要加密（个人信息、支付信息、）
