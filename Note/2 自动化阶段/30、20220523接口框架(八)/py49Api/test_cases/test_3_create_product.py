...
添加产品请求参数处理有问题，json对象处理的问题

["SELECT mobile_code FROM tz_sms_log WHERE user_phone='#mobile#'"]
...
{"expected_data":1,"actual_data":"SELECT COUNT(*) FROM tz_prod WHERE prod_id = (SELECT prod_id FROM  tz_sku WHERE party_code ='#partyCode#')"}
SELECT COUNT(*) FROM tz_prod WHERE prod_id = (SELECT prod_id FROM  tz_sku WHERE party_code ="1653308892310")
"""
import requests
import unittest
...
data_list = HandleExcel(file_name=data_dir,sheet_name="create_product").get_data()
